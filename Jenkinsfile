        ...
        stage('Push ee') {
         ...
         stage('deploy ee') {
            steps {
                ...
                APP=diplom_app
                TAG=$BUILD_ID
                ssh -o StrictHostKeyChecking=no  admin@62.84.114.157 -i /var/lib/jenkins/arch/diplom/id_rsa "sudo helm upgrade app /root/kuber/helm --set app_image=ruslan133/$APP:$TAG" 
                '''
